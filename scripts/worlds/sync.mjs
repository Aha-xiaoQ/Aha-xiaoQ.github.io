...
export function projectSnapshot(catalog,worlds){
 return {schemaVersion:1,revision:catalog.revision,updatedAt:catalog.updatedAt,guideHref:'/notes/mario-mix/docs/terra-stages/',starterHref:'/downloads/source/MarioMix_Worlds_W02_Starter.zip',instructionsHref:'/notes/mario-mix/docs/worlds-start/',source:'packages/mario-mix-worlds/content/catalog.json',levels:catalog.levels.map(l=>{const w=worlds.find(w=>w.level===l.id);return {id:l.id,title:l.title,characters:l.characters,status:l.status,summary:l.summary,playHref:l.playHref,introHref:l.introHref,templateStatus:l.template.status,templateHref:w?`/downloads/maps/MarioMix_Map_${l.id}_W02.zip`:null,missing:l.missing,owner:l.owner,issueUrl:l.issueUrl,areaCount:w?.coverage.areas??0,sectionViews:w?.coverage.sectionViews??0,previews:w?w.rooms.map(r=>({title:`${l.id} / ${r.roomId} · ${r.setting}`,src:`/assets/chapters/maps/${l.id}-${r.roomId}.svg`,width:r.map.width})):[]};})};
}
export function initializeProject(input){
 ...
 p.intro='四期已开放独立试玩：魂斗罗与洛克人、忍者与坦克、泰拉瑞亚，以及索尼克与奥日城堡关。关卡进度与基础地图模板在下方分别列出。';
 p.highlights=['1-1 至 1-4 四期独立试玩已上线；通用角色迁移分别验收。','八世界 32 关均有参考底图，按世界下载并独立管理。','水下、条件循环和正式角色仍需机制开发与原版核验。'];
