/** Read active HTML resource elements, excluding script data and foreign content.
 ...
 */
// Only the named character references needed by authored resource attributes are
// decoded here. Numeric references are decoded; unknown names remain literal.
export function decodeAttribute(value){
 return String(value).replace(/&(?:#x([0-9a-f]+);?|#([0-9]+);?|(?:amp|quot|apos|lt|gt|colon|sol|period|quest|equals|num|Tab|NewLine);)/gi,(all,hex,dec)=>{
  if(hex||dec){const cp=parseInt(hex||dec,hex?16:10);return cp===0||cp>0x10ffff||cp>=0xd800&&cp<=0xdfff?'\ufffd':String.fromCodePoint(cp);}
  const named={'&amp;':'&','&quot;':'"','&apos;':"'",'&lt;':'<','&gt;':'>','&colon;':':','&sol;':'/','&period;':'.','&quest;':'?','&equals;':'=','&num;':'#','&Tab;':'\t','&NewLine;':'\n'};
  return named[all]??all;
 });
}
export function parseAttributes(text){
 const attrs=Object.create(null),pattern=/([^\s=/>]+)(?:\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s>]+)))?/g;
 for(const m of text.matchAll(pattern)){const name=m[1].toLowerCase();if(!Object.hasOwn(attrs,name))attrs[name]=decodeAttribute(m[2]??m[3]??m[4]??'');}
 return attrs;
}
...
export function resourceElements(source){
 if(typeof source!=='string')throw new TypeError('HTML resource scan requires text');
 ...
 while(cursor<source.length){
  ...
  // from hiding a subsequent HTML resource after a normal body/div/p element.
  ...
  if(ns==='html'&&!inactive&&['script','link','base'].includes(name))elements.push({tag:name,attributes:parseAttributes(attributes),offset:start});
