...
## 2026-09-19 · 连续闯关离线版
用户要求全量连续关卡、自由选关、制作信息滚屏，并补充剩余生命与开屏选关。已接入编辑器“下载连续闯关版”；32 关 HTML 内嵌运行模块、图片及声音，开屏可选世界/关卡。过关传递生命、金币、分数、形态，关卡计时重新开始。HUD 显示 LIFE。最终制作滚屏含在下_小Q、GPT-6 Astra、项目和第三方素材署名。
源目录 D:/Code/xiaoq-homepage-push-r25；预览 D:/Code/xiaoq-homepage-github-main-r08-review，端口 4184。本轮未提交或推送。未改用户草稿。完整 32 关原版保真和逐关通关仍待实测，交付标注测试版。
...
## 2026-09-19 · 移除游玩 HUD 生命行
最新用户纠正：游戏左上角不需要 LIFE 常驻显示。移除该绘制行，保留关卡开场生命提示及实际生命逻辑。
...
## 2026-09-19 8-4 水下背景
- Underwater Castle误先匹配Castle黑色背景，改水下优先蓝色；普通Castle保持黑色。游泳/BGM本已有Underwater优先。真实area3/area4背景回归通过，已同步4184及重导离线包；未人工整关验收，未推送。


## 2026-09-19 HUD署名
- 白色Q由(3,16)移至(3,1)，10px；旁边(16,6)白色8px在下_小Q。分数栏不变。语法检查、离线生成打开通过；同步4184及Downloads离线包，未推送。
...
## 2026-09-20 16px字体优先与贴图
- 用户指定优先16px，已接入官方v2.502全字符集无损WOFF2并附许可。署名、结束/制作页默认16px族，小空间积分保留7px。新字体加载后按字形上边界定位署名，避免字体度量导致裁切。4184及离线包已同步。
- 人物起点预览原先固定马里奥，现读取playerSkin；导入结果在按钮旁保留。渲染分支mock检查通过，未完成真实选图浏览器复现。


## 2026-09-20 橡皮擦与手绘弹簧
- subtract保留sourceId，sourceType按sourceId找语义，避免切割后粉色缺图。新放spring固定16x29脚底对齐，作为完整组件擦除；工具栏显示完整弹簧。旧已丢失关联的碎片不能可靠自动还原，建议撤销后重擦。源逻辑检查、spring/playable检查完成；4184与离线已同步，未推送。


## 2026-09-20 署名缩小居中
- 用户纠正左上署名过大：该处改8px ChillBitmap小字库，Q仍16px；按Q alpha上下界与文字真实ascent/descent对齐垂直中心。其他有空间提示保留16px族。Edge截图复核、离线导出打开通过，4184和Downloads当天离线包已同步。


## 2026-09-20 署名顶部留白
- Q和署名整体下移3个游戏像素，保留实际字形垂直中心对齐。语法、Edge截图捕获、离线导出打开通过；同步4184和当天Downloads离线包。


## 2026-09-20 HUD水平对齐
- 根据用户标注：署名真实左边界对齐MARIO x24；Q alpha可见边界中心对齐左侧0..24区域中心x12，保留垂直居中与顶部3px偏移。Edge截图复核及离线导出完成，同步4184和Downloads当天包。


## 2026-09-20 过关生命数字号
- level-intro独立样式，标题及生命32px ChillBitmap16，短屏24px，避免试玩父级小字继承。离线Edge初始与嵌套容器computedStyle均32px验证通过，非实际整关通关。已同步4184及当天离线下载包。


## 2026-09-20 Warp管道数字
- drawReference对world-transport绘制目标世界数字，读取transport.map，使用HUD同款像素字，放管道中心上方24px。1-2与4-2所有标记绘制检查通过；4184及当天离线包已同步。仅补显示，未声称跨世界传送逻辑验收。


## 2026-09-20 Warp跳关连接
- world-transport补入管道动作/56帧入管后warpTarget。play onWarp传递carry及session；连续版按目标索引run，编辑器按目标加载底图到同session，不改变草稿。单关离线没有目标数据，allowWarp关闭。
- 1-2三根真实管道4-1/3-1/2-1下键入管、目标与生命回归通过，共11项路径测试。未完成人工三路线整关跨关试玩；已同步4184与当天全量离线包。


## 2026-09-20 3-1及隐藏出口检查
- 3-1横管误写entrance1无transport，7-1遗漏transport；按已知level补area1->area0 entrance1，旧工程projectPack克隆和新底图均生效，不覆盖已有出口。
- 扫描32关基础区域19处PipeHorizontal，小/大形态共38次入管到出场结束回放全通过。测试从管口开始，不代表全部隐藏区域人工全流程（包括藤蔓/天空）通过。之前180帧持续向右会在出口后走入敌人导致4例死亡，改为以出管完成作为断言边界，未更改敌人机制。另16项路径/导出回归通过。已同步4184及当天Downloads全量离线包，未推送。


## 2026-09-20 藤蔓天空闭环
- 藤蔓原先误走蘑菇分支。新增逐帧生长、↑攀爬、跳跃/横向脱离，爬顶进入目标区域；2-1/3-1/5-2/6-2天空掉落按topology.exit返回xloc且不扣命，载人平台启动右行。4-2藤蔓通选关区，source-transport-not-simulated世界管道显示与跳关也识别。web/手柄/触屏上方向输入接入；源骨架默认spawn进天空，未实现完整原版进场演出。
- 五入口受控顶砖+攀爬+回程回归通过，合计122项；Edge3-1添加测试支撑台后真实空格顶砖长藤蔓，vine-preview.png目视通过，不代表原关完整可达性验收。网页版与离线重新导出打开通过。待继续真人流程/藤蔓上下移动及天空平台全程视觉验收，未推送。


2026-09-20 4-4 retry correction: section-fail sensors now latch a wrong choice within their finite bounds; retry deferred to corridor end/closing wall. Preserve horizontal velocity, legal vertical position and player screen X. 92 castle-route/engine checks pass. Preview mirror and offline download regenerated. This is NOT proof of seamless original-style scenery streaming: retries still relocate world coordinates and scenery can change; no full manual 4-4 traversal captured. Next: replace fixed-route retry with continuous repeated-section generation and verify moving viewport continuity. HUD branding now Q + 在下小Q · 非官方同人.

4-4 geometry follow-up: NES map comparison https://www.mariowiki.com/File:SMB_NES_World_4-4_Map.png shows continuous upper floor after pipe; transcription leaves erroneous x976..1056 gap. Correct r016 width272->352 in template, compiler and exact-geometry old-draft repair. Actual motor walking/running x920->1120 stays on y96 without retry; 15 route tests pass. Preview screenshot castle-corridor.png inspected; source/preview/offline updated. Other generated SVG/ZIP derivatives not rebuilt; whole-level traversal and seamless wrong-route scenery still outstanding.

Hidden exit audit 2026-09-20: expanded hidden-exits.test.mjs to include first-world semantics.type (previous audit omitted these), all three Mario forms, completed exit animation and post-exit movement. 23 side exits x3=69, four sky returns x3=12 with real motor/gravity and safe landing/movement, seven warp destinations=7: 88/88 pass. Existing five vine growth/return state tests also passed. No runtime changes needed. Tests initialize beside exit or in sky gap; not full room traversal nor browser/audio visual certification. Warp tests prove target dispatch only, not campaign next-session loading.

8-4 terminal dead-end repair: add explicit return portals to area0/r020, area1/r022, area2/r019 targeting area0/arrival-location-1. Finite-map adaptation requested by user, not a claim of original seamless section-loop behavior. Applied in fromTemplate/projectPack repair for saved drafts. Added 30 real-motor vertical pipe tests (10 connections x3 forms), including same-room returns, completed transitions and onward motion; all 118 hidden-exit tests pass. Preview and offline synced.


## 2026-09-20 GitHub checkpoint scope
User requests today’s work pushed, then explicitly excludes the buggy Contra 1-2 prototype. Commit only the Mario workshop, gameplay/map fixes, offline options and supporting assets/tests. Prototype-only module/tests and shared-file changes are preserved locally in C:/Users/Bright/AppData/Local/Temp/mario-bill-local-only-20260920 and restored after commit; preview mirror and Downloads demo remain local. No Contra selector, module, audio mapping or combat branch is included in this checkpoint.
