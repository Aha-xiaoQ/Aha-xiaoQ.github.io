...
维护者负责玩法方向、整体审美、合并和发布；贡献者负责小范围源码、测试和资料；审阅者检查范围、接口、回归和资源权利。架构登记中的 reviewerRole 是职责分类，不是假装已有三位团队成员。
...
发布流程：确认原始底稿不变 → verify → 必要人工验收 → pack → 解压源码复测 → 更新网站源码 ZIP/JSON/资料页 → 审阅 diff → 用户手动推送。任何新编辑后旧候选 SHA 的验收必须重新评估。
