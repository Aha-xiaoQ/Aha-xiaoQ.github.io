import {selectHero} from './editor-heroes.mjs';
...
export async function buildCampaign(read=async path=>{const response=await fetch(new URL(path,import.meta.url));if(!response.ok)throw Error('无法读取 '+path);return response.json();},progress=()=>{},options={}){
 const character=await read('./runtime/character.json'),levels=[];
 for(let world=1;world<=8;world++)for(let number=1;number<=4;number++){
  const id=world+'-'+number,template=await read(id==='1-1'?'./classic-1-1.json':'../generated/levels/'+id+'/template.json'),doc=selectHero(fromTemplate(template),options.hero||'mario');
