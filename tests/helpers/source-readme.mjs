...
export function assertSourceReadme(text,project,file='README'){
 ...
 const links=[...html.matchAll(/<a href="([^"]+)">([\s\S]*?)<\/a>/g)].map(m=>({href:m[1],label:m[2].replace(/<[^>]+>/g,'').trim()}));
 const expected=requiredSourceLinks(project);
 for(const target of expected){
  const found=links.filter(l=>l.href===target.href);
  assert.equal(found.length,1,`${file}: expected one visible ${target.kind} link: ${target.href}`);
