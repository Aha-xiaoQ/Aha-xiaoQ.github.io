...
test('single shared game category does not produce redundant filters',()=>{const c=collectionContext(),s=c.SITE_WORKSHOP.renderCollection('games',c.SITE_DATA,c.SITE_TAXONOMY);assert.ok(!s.includes('data-workshop-filter'));assert.ok(s.includes('data-filter-summary'));const games=c.SITE_DATA.items.filter(x=>x.primaryType==='game'&&x.visibility==='public'&&x.lifecycleStatus!=='archived');assert.ok(games.length>0);assert.equal((s.match(/role="listitem"/g)||[]).length,games.length);for(const item of games)assert.equal(s.split('data-content-id="'+item.id+'"').length-1,1);});
