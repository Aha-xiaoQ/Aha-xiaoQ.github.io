import {installWallI18n, translate} from './wall-i18n.mjs?v=20260907-guestbook-r2';
import { contentBounds, fitCamera, expandWall } from './wall-geometry.mjs?v=20260907-guestbook-r2';
import { installWallSkin } from './wall-skin.mjs?v=20260907-guestbook-r2';
import { installNoteDrag } from './note-drag.mjs?v=20260907-guestbook-r2';
...
if(cloudMode){
  ...
  async function load(reset,requestedOwn=own){
    ...
    try{
      const {loadCloudPage}=await import('./cloud-preview.mjs?v=20260907-guestbook-r2');
  ...
  async function sendAt(x,y,rollback=null){
    ...
    try{
      const {getCloudClient}=await import('./cloud-preview.mjs?v=20260907-guestbook-r2');const api=await getCloudClient();
  ...
  retryButton.onclick=async()=>{
    ...
    let success=false;
    try{const {getCloudClient}=await import('./cloud-preview.mjs?v=20260907-guestbook-r2');await (await getCloudClient()).submit(lastSubmission);if(text.value===lastSubmission.body){text.value='';$('counter').textContent='0 / 500 字';}lastSubmission=null;uncertain=false;success=true;}
  ...
  async function saveEdit(){
    ...
    let ok=false;
    try{const {getCloudClient}=await import('./cloud-preview.mjs?v=20260907-guestbook-r2');await (await getCloudClient()).edit(target.remoteId,target.revision,draft);ok=true;}
  ...
  $('remove').onclick=async()=>{
    ...
    try{const {getCloudClient}=await import('./cloud-preview.mjs?v=20260907-guestbook-r2');if(!await (await getCloudClient()).remove(target.remoteId))throw Error('NOT_OWNED');notes.delete(target.id);target.el.remove();select(null);if(editTarget)endEdit();$('status').textContent='便签已删除，占位已释放。';}
