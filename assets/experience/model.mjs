...
export function search(data,{q='',type='all',page=1,size=8}={}){
 ...
 const scored=data.entries.filter(e=>type==='all'||!TYPES[type]||e.type===type).map(e=>{const title=normalize(e.title),tags=normalize(e.tags.join(' ')),body=normalize(e.summary),all=title+' '+tags+' '+body;if(!terms.every(t=>all.includes(t)))return null;const score=!query?({game:50,project:40,tool:30,note:20,guide:10}[e.type]||0):(title===query?150:0)+(title.startsWith(query)?60:0)+(title.includes(query)?30:0)+terms.reduce((s,t)=>s+(title.includes(t)?15:tags.includes(t)?6:1),0);return {e,score};}).filter(Boolean).sort((a,b)=>b.score-a.score||a.e.id.localeCompare(b.e.id));
