import {isPublicUpdate} from './update-audience.mjs?v=dev-r44-55de28a9c31270b5';
import {developmentNav,labSpotlight,projectNavigation,projectOverview,projectDocument,documentDirectory} from './public-layout.mjs?v=dev-r44-55de28a9c31270b5';
import {renderMarkdown} from './markdown.mjs?v=docs-r26';
import {readingLink,readingDocument} from './documents.mjs?v=dev-r44-55de28a9c31270b5';
...
function indexPage(projects,notes){
 ...
 return `${developmentNav('projects')}<section class="section">${sectionHead('PROJECTS','项目进展')}<div class="j-toolbar"><div class="filter-bar j-project-filter" aria-label="项目类型"><button type="button" data-category="all" aria-pressed="true">全部</button>${cats.map(c=>`<button type="button" data-category="${c}" aria-pressed="false">${CATEGORY_LABELS[c]}</button>`).join('')}${projects.some(p=>p.visibility==='archived')?'<button type="button" data-category="archived" aria-pressed="false">归档</button>':''}</div><label class="j-search"><span class="j-search-label">搜索项目</span><input type="search" data-project-search placeholder="项目名称或关键词" maxlength="100" enterkeyhint="search"></label></div><div class="j-project-results-meta"><span role="status" aria-live="polite" aria-atomic="true" data-project-result-status>显示 ${selectProjects(projects).count} 个项目</span><button type="button" class="j-clear-filter" data-clear-project-filters hidden>清除筛选</button></div><div class="j-project-grid" data-project-results>${renderProjectCards(projects)}</div></section><section class="section">${sectionHead('02 / UPDATES','最近有什么变化','查看新功能、问题修复和开发进展。')}<div data-update-preview>${updateRows(projects,{limit:2,audience:'public'})}</div><div class="j-section-footer">${link('全部更新','/notes/updates/')}</div></section>${notesSection(notes)}${labSpotlight()}<section class="section"><div class="entry j-invite"><div><p class="eyebrow">MAKE SOMETHING TOGETHER</p><h2>一起改进作品</h2><p>发现一个问题，分享一次试玩，或把一处细节做得更好。从你擅长的事开始。</p></div>${link('参与方式','/notes/contribute/','button button--quiet')}</div></section>`;
}
function navigation(p,r){
 const custom=projectNavigation(p,r);if(custom!==null)return custom;
